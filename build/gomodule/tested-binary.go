...
var (
	...
	pctx = blueprint.NewPackageContext("github.com/Encelad/archlab2bood/gomodule")
...
func (gb *goBinaryModuleType) GenerateBuildActions(ctx blueprint.ModuleContext) {
	...
	outputFile := path.Join(config.BaseOutputDir, "bin", gb.properties.OutTestFile)
	var inputs []string
	inputErors := false
